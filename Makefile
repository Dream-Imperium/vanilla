...
# HEADERS RULES
inc/vanilla/cpu.h: config.mk sys/$(OSNAME)/common.h.in sys/$(OSNAME)/$(OBJTYPE).h.in
...
# USER ACTIONS
install: all
	$(INSTALL) -dm 755 $(DESTDIR)$(INCDIR)/vanilla
	$(INSTALL) -dm 755 $(DESTDIR)$(LIBDIR)
	$(INSTALL) -cm 644 $(HDR) $(DESTDIR)$(INCDIR)/vanilla
	$(INSTALL) -cm 644 $(LIB) $(DESTDIR)$(LIBDIR)

clean:
	rm -f $(BIN) $(OBJ) $(LIB)
	rm -f inc/vanilla/cpu.h
